  ...
  for(b=i-1;b<i+2;b++){
    if(b>=0 && b< x ){/*au cas on est dans les bord ==> on peut pas acceder à certain espace de la matrice grille*/
      ...
      //matrice 3*3 de centre i,j 
        if(a>=0 && a<y){
          if(*(grille+(y*b)+a)==alive) nbv++;//cellule vivant sur la matrice 3*3 de centre i,j => nombre des voisins +1
        }
      }
    }
  }
  if(*(grille+(y*i)+j)==alive) nbv--;// au cas où la cellule d'indice i,j est aussi vivant,on soustri 1 de nbv 
...
void Voisins(Donnes *d){
  ...
  for(i=0;i<x;i++){
    for(j=0;j<y;j++){
      *(voisin+(y*i)+j)=nbDeVoisins(grille,i,j);//voisin[i][j]=nombre des voisins de la cellule i,j
    }
  }
}
  ...
  for(i=0; i< x; i++){
    for(j=0;j<y;j++){
      if(*(grille+(i*y)+j)==alive){
        if(*(voisin+(i*y)+j)==2||*(voisin+(i*y)+j)==3) *(grille+(i*y)+j)=alive;/*2 voisins ou 3 voisin => l'individu est vivant*/  
        else *(grille+(i*y)+j)=dead;// sinon il meurt
      }
      else {
        if(*(voisin+(i*y)+j)==3) *(grille+(i*y)+j)=alive;/*si il etait mort et il a 3 voisin ==> rendre vivant*/   
      }
    }
  }
}
  ...
  for(i=0; i< x; i++){
    for(j=0;j<y;j++){
      ...
      //si l'individu est mort
        if(*(voisin+(i*y)+j)==3||*(voisin+(i*y)+j)==6||*(voisin+(i*y)+j)==7||*(voisin+(i*y)+j)==8) *(grille+(i*y)+j)=alive;/*si il etait mort et il a 3,6,7,8 voisins ==> rendre vivant*/            
      }
    }
  }
}
...
void fileToGrid(Donnes *d,FILE *in){
  ...
  for(int i=0;i<x;i++){
    for(int j=0;j<y;j++){
      if(Tab[i][j]=='0') *(population+i*y+j)=dead;// si le caractère lu à l'indice i,j est '0'==> cellule est meurt
      if(Tab[i][j]=='1') *(population+i*y+j)=alive;//  si le caractère lu à l'indice i,j est '1'==> cellule vivant
    }
  }
}
/*
